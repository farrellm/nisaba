...
- Response processing lives in `internal/handler/response.go`: `parseTopLevelTags` byte-scans free-form responses (not `encoding/xml`) for top-level tags, counting same-name nesting depth and degrading gracefully on malformed input. `RunBlock` merges the parsed tags (plus the `output` key when set) via `MergeDocumentAttributes`. Covered by `response_test.go` (run with `make backend-test`; see also `internal/llm/names_test.go`)
