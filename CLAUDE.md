...
Quick checks: `gofmt -l backend/` (format), `cd frontend && npx tsc --noEmit` (typecheck only). There is no separate frontend lint step. The frontend tsconfig targets ES2020 (`lib: ES2020`), so newer stdlib like `String.replaceAll`/`Array.at` isn't typed — use ES2020-compatible equivalents.
